...
class Panel_Window_Pago(ft.View):
    ...
    def panelViewVolver(self,e):
        self.page.views.pop()  
        self.page.views[-1].inicial()
        self.page.views[-1].cargarProductos()
        self.page.views[-1].cargarCategorias()
        self.page.views[-1].cargarMarcas()
    ...
    def clickBtnComprar(self,e):
        ...
        if stock != "" and formaPago != "" and direccionPedido != "":
            if int(stock) <= stockDisponible:
                if comprarProducto({"id_pedido":idPedido,"nombre_producto":nombreProducto,'precio':float(precioProducto),'stock':int(stock),'forma_pago':formaPago
                                ,"usuario_cliente":nombreCliente,"direccion_pedido":direccionPedido}):
